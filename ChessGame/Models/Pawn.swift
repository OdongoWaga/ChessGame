...
class Pawn: Piece {
    ...
    static let doubleMoveAllowedPositionsForWhitePawns = DirectedPosition(position: Position(rank: .A, row: .two), perspective: .white).rightSpaces.map { 
        $0.position
    } + [Position(rank: .A, row: .two)]
    static let doubleMoveAllowedPositionsForBlackPawns = DirectedPosition(position: Position(rank: .A, row: .seven), perspective: .black).leftSpaces.map {
        $0.position
    } + [Position(rank: .A, row: .seven)]
    
    static let promotionRequiredPositionsForWhitePawns = DirectedPosition(position: Position(rank: .A, row: .eight), perspective: .white).rightSpaces.map {
        $0.position
    } + [Position(rank: .A, row: .eight)]
    static let promotionRequiredPositionsForBlackPawns = DirectedPosition(position: Position(rank: .A, row: .one), perspective: .black).leftSpaces.map {
        $0.position
    ...
    override func possibleMoves(position: Position, game: Game) -> Set<Move> {
        ...
        let allMovePositions = [frontMove, frontLeftMove, frontRightMove].compactMap({
            $0
        })
        ...
        let enPassantMove = { () -> Move? in
            ...
            case .white:
                ...
                guard Pawn.doubleMoveAllowedPositionsForBlackPawns.contains(lastMove.origin), abs(lastMove.origin.row.rawValue - lastMove.destination.row.rawValue) == 2 else {
                    return nil
                }
                
            case .black:
               
                guard Pawn.doubleMoveAllowedPositionsForWhitePawns.contains(lastMove.origin), abs(lastMove.origin.row.rawValue - lastMove.destination.row.rawValue) == 2 else {
                    return nil
                }
            }
    ...
    override func threatenedPositions(position: Position, game: Game) -> BooleanChessGrid {
        ...
        return BooleanChessGrid(positions: [frontLeftThreat, frontRightThreat].compactMap({
            $0
        }))
